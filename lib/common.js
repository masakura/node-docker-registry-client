...
/**
 * Split part names: [NAMESPACE]/[REPO]
 *
 * @param source {String} Target name
 * @return {Array.<String>} Name parts
 */
function splitNameParts(source) {
    var items = strsplit(source, '/');

    if (items.length >= 2) {
        var repo = items.pop();
        return [items.join('/'), repo];
    }
    return items;
}
...
function parseRepo(arg, defaultIndex) {
    ...
    var nameParts = splitNameParts(remoteName);
    var ns, name;
